...
import { ArrowUpRight, Check, Clock, MapPin, UsersRound } from "lucide-react-native";
    ...
    <View style={styles.card}>
      ...
      <View style={styles.footer}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            styles.action,
            isGoing && styles.actionGoing,
            isWaitlisted && styles.actionWaitlisted,
            pressed && { opacity: 0.7 },
        ...
        >
          <Text
            style={[
              styles.actionText,
              isGoing && styles.actionGoingText,
              isWaitlisted && styles.actionWaitlistedText,
            ]}
          >
            {actionLabel}
          </Text>
          {isGoing ? (
            <Check size={15} color={brand.primaryStrong} />
          ) : isWaitlisted ? (
            <Clock size={15} color={brand.accentStrong} />
          ) : (
            <ArrowUpRight size={15} color={brand.primaryStrong} />
          )}
        </Pressable>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  priceGroup: { gap: 4 },
  price: { color: brand.ink, fontFamily: brand.fonts.bold, fontSize: 15 },
  ...
  actionGoing: { backgroundColor: brand.primarySoft },
  actionWaitlisted: { backgroundColor: "rgba(201,110,78,0.12)" },
  actionText: { fontFamily: brand.fonts.bold, fontSize: 11, color: brand.primaryStrong },
  actionGoingText: { color: brand.primaryStrong },
  actionWaitlistedText: { color: brand.accentStrong },
  skeleton: { gap: 14 },
  block: { backgroundColor: brand.mutedSurface, borderRadius: 6 },
});
